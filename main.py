...
BOT_DESCRIPTION = (
    ...
    "Если в переписке удалили текст, фото, видео, кружок или голосовое, бот сохранит копию "
    "и пришлёт её вам. Также он показывает изменённые сообщения и помогает выгружать диалоги.\n\n"
    "Подключение занимает пару минут. Первые 7 дней бесплатно."
)
TRIAL_DAYS = 7
PRO_TARIFFS = {
    "week": ("Неделя", 99),
...
def _start_text(user_id: int | None, status: str) -> str:
    ...
    return (
        "🕵️ <b>Major Spy</b>\n"
        "<i>Помогает не потерять важные сообщения</i>\n\n"
        "Если собеседник удалит текст, фото, видео, кружок или голосовое, "
        "бот сохранит копию и пришлёт её вам. Ещё он показывает, что именно изменили в сообщении.\n\n"
        "<b>Что умеет</b>\n"
        "• сохраняет удалённые сообщения\n"
        "• сохраняет фото, видео, кружки и голосовые\n"
        "• показывает, что было до изменения\n"
        "• выгружает нужный диалог в файл\n"
        "• помогает с переводом и раскладкой текста\n\n"
        "<b>Как начать</b>\n"
        "Нажмите <b>Подключить</b> и добавьте бота в настройках Telegram. "
        f"После подключения включится пробный период на <b>{TRIAL_DAYS} дней</b>.\n\n"
        f"<b>ID:</b> <code>{user_id}</code>\n"
        f"<b>Доступ:</b> {escape(status)}\n"
        f"<b>Major Pro:</b> {escape(subscription_text)}"
    )
